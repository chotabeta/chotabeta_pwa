			...
			<q-page v-if="NoOfItemsInCart != 0" class="bg-orange-1 q-px-sm">
				<div class="row">
					<div class=" col-12 swipe-container" v-for="item in items_data">
						...
						<q-btn label="delete" id="swipe_delete" class="text-white action bg-red q-ml-sm cb-round-borders-20"></q-btn>
					</div>
				</div>
				<div class="row absolute-bottom q-ma-sm">

					<q-btn class="bg-green-8 full-width cb-round-borders-10 text-white cb-font-16" @click="$router.push('food-checkout?plan=')" v-if="check_schedule == 1">checkout<q-icon name="currency_rupee" class="q-pl-md" size="16px" ></q-icon>{{ total_mrp }} </q-btn>
