     ...
     <div class="row items-center justify-center">
        ...
        <q-card style="border:1px solid grey" class="cb-round-borders-10 cb-shadow-1">
          ...
          <q-card-section class="cb-font-16 text-grey-9">
            <div class="flex q-pt-xs">Trip Type<q-space></q-space><span v-if="trip_type == 0 || trip_type == '0'">One Way</span>
                          <span v-if="trip_type == 1 || trip_type == '1'">Round Trip</span></div>
  ...
  methods:{
    ...
    placeorder(){
      ...
      if(ps.payment == 'Cash On Delivery'){var payment = 'COD';}
               ...
               ps.$q.notify({ message: "Pay Now Online is Not Available! Please Try Another Method", }); exit; }

      if(ps.vehicle_type == 0 || ps.vehicle_type == '0'){
        var car_type = "Automatic";

      }else if (ps.vehicle_type == 1 || ps.vehicle_type == '1'){
        var car_type = "Manual";
      }
  ...
  if(call_from == 1){
   ...
   var pickup_address = JSON.parse(localStorage.getItem('pickup_address'));
    ...
    let config = { headers: { "Authorization": `Bearer ${ps.access_token}`,}}
      ...
      // if(ps.delivery_address_array.location == ps.pickuplocation_array.location ){
